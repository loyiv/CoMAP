...
def _consume_agent_step_metadata(agent: agents.LMAgent) -> dict[str, Any] | None:
    consume = getattr(agent, "consume_step_metadata", None)
    if callable(consume):
        return consume()
    return None
...
def interactive_loop(
    ...
    while not state.finished:
        ...
        try:
            ...
            step_metadata = _consume_agent_step_metadata(agent)
        ...
        if step_metadata is not None:
            state.step_metadata.append(step_metadata)
