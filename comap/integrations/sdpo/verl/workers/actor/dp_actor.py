...
from verl.trainer.ppo.core_algos import (
    ...
    compute_supervised_action_loss,
    get_policy_loss_fn,
    kl_penalty,
)
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        auxiliary_supervision_enabled = bool(
            self_distillation_enabled
            and self_distillation_cfg is not None
            and self_distillation_cfg.get("enable_auxiliary_supervision", False)
            and "supervised_input_ids" in data.batch.keys()
        )
        auxiliary_supervision_required_keys = {
            "supervised_input_ids",
            "supervised_attention_mask",
            "supervised_position_ids",
            "supervised_responses",
            "supervised_response_mask",
            "supervised_reflect_mask",
            "supervised_draft_mask",
            "supervised_anchor_mask",
            "supervised_fallback_mask",
        }
        if auxiliary_supervision_enabled:
            assert auxiliary_supervision_required_keys.issubset(set(data.batch.keys())), (
                f"Missing required keys: {auxiliary_supervision_required_keys - set(data.batch.keys())}"
            )
        ...
        if auxiliary_supervision_enabled:
            select_keys.extend(list(auxiliary_supervision_required_keys))
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    auxiliary_supervision_loss = None
                    if auxiliary_supervision_enabled:
                        supervised_inputs = {
                            "responses": model_inputs["supervised_responses"],
                            "input_ids": model_inputs["supervised_input_ids"],
                            "attention_mask": model_inputs["supervised_attention_mask"],
                            "position_ids": model_inputs["supervised_position_ids"],
                        }
                        supervised_outputs = self._forward_micro_batch(
                            supervised_inputs,
                            temperature=temperature,
                            calculate_entropy=False,
                        )
                        supervised_log_prob = supervised_outputs["log_probs"]
                        supervised_response_mask = model_inputs["supervised_response_mask"]
                        origin_specs = [
                            (
                                "reflect",
                                model_inputs["supervised_reflect_mask"],
                                float(self_distillation_cfg.get("reflect_main_loss_weight", 0.0)),
                            ),
                            (
                                "draft",
                                model_inputs["supervised_draft_mask"],
                                float(self_distillation_cfg.get("draft_distill_loss_weight", 0.0)),
                            ),
                            (
                                "anchor",
                                model_inputs["supervised_anchor_mask"],
                                float(self_distillation_cfg.get("expert_anchor_loss_weight", 0.0)),
                            ),
                            (
                                "fallback",
                                model_inputs["supervised_fallback_mask"],
                                float(self_distillation_cfg.get("transition_fallback_loss_weight", 0.0)),
                            ),
                        ]
                        for origin_name, sequence_mask, origin_weight in origin_specs:
                            if origin_weight <= 0:
                                continue
                            origin_loss = compute_supervised_action_loss(
                                log_prob=supervised_log_prob,
                                response_mask=supervised_response_mask,
                                sequence_mask=sequence_mask,
                                loss_agg_mode=loss_agg_mode,
                                config=self.config,
                            )
                            if auxiliary_supervision_loss is None:
                                auxiliary_supervision_loss = origin_loss * origin_weight
                            else:
                                auxiliary_supervision_loss = auxiliary_supervision_loss + (origin_loss * origin_weight)
                            micro_batch_metrics[f"actor/aux_supervision/{origin_name}_loss"] = origin_loss.detach().item()
                            micro_batch_metrics[f"actor/aux_supervision/{origin_name}_weight"] = origin_weight
                            micro_batch_metrics[f"actor/aux_supervision/{origin_name}_fraction"] = sequence_mask.float().mean().item()
                    ...
                    policy_loss = pg_loss
                    if auxiliary_supervision_loss is not None:
                        policy_loss = policy_loss + auxiliary_supervision_loss
                        micro_batch_metrics["actor/aux_supervision/total_loss"] = auxiliary_supervision_loss.detach().item()
