...
class SelfDistillationConfig(BaseConfig):
    ...
    Args:
        ...
        enable_auxiliary_supervision (bool): Whether to add supervised action losses on top of sdpo loss.
        reflect_main_loss_weight (float): Weight for reflect-online supervised loss.
        draft_distill_loss_weight (float): Weight for draft-distill supervised loss.
        expert_anchor_loss_weight (float): Weight for expert-anchor supervised loss.
        transition_fallback_loss_weight (float): Weight for fallback transition supervised loss.
    """
    ...
    enable_auxiliary_supervision: bool = False
    reflect_main_loss_weight: float = 0.0
    draft_distill_loss_weight: float = 0.0
    expert_anchor_loss_weight: float = 0.0
    transition_fallback_loss_weight: float = 0.0
    ...
    def __post_init__(self):
        ...
        for field_name in (
            "reflect_main_loss_weight",
            "draft_distill_loss_weight",
            "expert_anchor_loss_weight",
            "transition_fallback_loss_weight",
        ):
            field_value = getattr(self, field_name)
            if field_value < 0:
                raise ValueError(f"self_distillation.{field_name} must be >= 0, got {field_value}")
